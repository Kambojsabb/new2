...
function addBackButton(onClickCallback) {
  ...
  backButton.style.position = 'absolute';  // Pin the button to the bottom
  backButton.style.bottom = '100px';  // Space from the bottom of the container
  backButton.style.left = '50%';  // Center the button horizontally
  backButton.style.transform = 'translateX(-50%)'; // Ensure it is exactly centered
  backButton.style.transition = 'transform 0.2s ease-in-out';  // Smooth animation for both hover and click
  
  // Hover effect - button grows when hovered
  backButton.onmouseenter = () => {
    backButton.style.transform = 'translateX(-50%) scale(1.1)';  // Grow the button to 110% of its size
  };

  // Mouse leave effect - button returns to normal size when cursor leaves
  backButton.onmouseleave = () => {
    backButton.style.transform = 'translateX(-50%) scale(1)';  // Return to original size
  };

  // Click effect - button shrinks when clicked
  backButton.onclick = () => {
    backButton.style.transform = 'translateX(-50%) scale(0.9)';  // Shrink the button to 90% of its size

    // After animation completes, execute the onClickCallback
    setTimeout(() => {
      onClickCallback();
      // Reset the button's size to normal after the callback (optional)
      backButton.style.transform = 'translateX(-50%) scale(1)';
    }, 200);  // Wait for the shrinking animation to finish (200ms)
  };
...
function createHackNote() {
  ...
  setTimeout(showSuccessfullyCreatedMessage, 2000); // Show "Successfully Created" after 2 seconds
  addBackButton(() => {
...
function downloadSilentAudio() {
  ...
  offlineAudioContext.startRendering().then((renderedBuffer) => {
    ...
    downloadLink.download = 'silent_hack_note.wav'; // Name of the silent audio file
  });
}
...
function displayQrPage() {
  ...
  qrCode.src = 'qr.png.jpg'; // QR code path
  qrCode.alt = 'QR Code';
